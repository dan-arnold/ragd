...
/// Dependency-manager lockfiles: valid (often huge) text, but
/// machine-generated package/version/checksum listings with essentially no
/// value for code search or Q&A -- and, being regenerated by routine
/// `build`/`install` commands, disproportionately expensive to index
/// relative to that value. Matched by exact filename rather than
/// extension: most of these don't share a common one (`package-lock.json`,
/// `pnpm-lock.yaml`), and a bare `.lock` extension would be both too broad
/// (any unrelated file that happens to end in `.lock`) and too narrow
/// (misses those two).
...
/// Whether `file_name` (the final path component, e.g. from
/// [`std::path::Path::file_name`]) is a known dependency-manager lockfile.
...
mod tests {
    ...
    #[test]
    fn is_lockfile_name_matches_known_lockfiles_only() {
        assert!(is_lockfile_name("Cargo.lock"));
        assert!(is_lockfile_name("package-lock.json"));
        assert!(is_lockfile_name("yarn.lock"));
        assert!(
            !is_lockfile_name("cargo.lock"),
            "matching is exact, not case-insensitive -- these filenames have one canonical casing per tool"
        );
        assert!(!is_lockfile_name("Cargo.toml"));
        assert!(!is_lockfile_name("mylockfile.lock"));
    }
